...
const Index = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      submitForm(userInfo, yearId, formData, image, "update").then(() =>
  ...
  React.useEffect(() => {
    ...
    getYearlyData({ student_id: userInfo._id, year_id: yearId })
      .then((res) => {
        const yearlyData = res?.students?.[0]?.yearly_data?.[0];
        const student = res?.students?.[0];

        if (yearlyData && student) {
          formik.setValues({
  ...
  }, [yearId, userInfo?._id, getYearlyData]);
